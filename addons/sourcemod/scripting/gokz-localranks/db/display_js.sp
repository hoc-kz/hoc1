...
void DB_OpenJumpStats(int client, int targetSteamID, int mode, int blockType)
{
	...
	if (blockType == 0)
	{
		FormatEx(query, sizeof(query), sql_jumpstats_getpbs, targetSteamID, mode, targetSteamID, mode);
	}
	else
	{
		FormatEx(query, sizeof(query), sql_jumpstats_getblockpbs, targetSteamID, mode, targetSteamID, mode);
	}
	txn.AddQuery(query);
...
public void DB_TxnSuccess_OpenJumpStats(Handle db, DataPack datapack, int numQueries, Handle[] results, any[] queryData)
{
	...
	if (blockType == 0)
	{
		...
		FormatEx(buffer, sizeof(buffer), "%T", "Jump Stats - Jump Console Header", client, gC_ModeNames[mode], alias, targetSteamID & 1, targetSteamID >> 1);
		...
		{
			int jumpType = SQL_FetchInt(results[1], 0);
			float distance = SQL_FetchFloat(results[1], 1) / GOKZ_DB_JS_DISTANCE_PRECISION;
	...
	else
	{
		...
		FormatEx(buffer, sizeof(buffer), "%T", "Jump Stats - Block Jump Console Header", client, gC_ModeNames[mode], alias, targetSteamID & 1, targetSteamID >> 1);
		PrintToConsole(client, "%s", buffer);
		int titleLength = strlen(buffer);
		strcopy(buffer, sizeof(buffer), "----------------------------------------------------------------");
		buffer[titleLength] = '\0';
		...
		while (SQL_FetchRow(results[1]))
		{
			...
			float distance = SQL_FetchFloat(results[1], 2) / GOKZ_DB_JS_DISTANCE_PRECISION;
			int strafes = SQL_FetchInt(results[1], 3);
